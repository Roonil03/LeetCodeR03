...
Some of the grid squares are obstacles. The ith obstacle is at grid point `obstacles[i]` = `(x`<sub>`i`</sub>`, y`<sub>`i`</sub>`)`. If the robot runs into an obstacle, it will stay in its current location (on the block adjacent to the obstacle) and move onto the next command.
